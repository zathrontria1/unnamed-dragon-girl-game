...
uint16_t ani_fixedspr_frame_coin;
uint8_t * ani_fixedspr_addr_coin;
...
void AniSystem_Spr_UpdateFixedTiles()
{
    ...
    {
        ani_fixedspr_frame_coin++;
    }

    uint16_t temp_frame;

    if (ani_fixedspr_frame_coin >= 11)
    {
        ani_fixedspr_frame_coin = 0;
        temp_frame = 0;
    }
    else if (ani_fixedspr_frame_coin >= 6)
    {
        temp_frame = 11 - (ani_fixedspr_frame_coin + 1);
        ani_coin_flip = true;
    }
    else
    {
        temp_frame = ani_fixedspr_frame_coin;
        ani_coin_flip = false;
    }
    ...
    if (temp_src != ani_fixedspr_addr_coin)
    {
        if (!DmaSystem_AddItemToQueue(temp_src, temp_dest, 128, VRAM_INCHIGH, 1))
        {
            ani_fixedspr_addr_coin = temp_src;
        }
    }
