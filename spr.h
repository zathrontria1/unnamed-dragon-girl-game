extern ZP uint16_t spr_sprite_count; // Rendered sprites this frame
extern uint16_t spr_sprite_count_prev; // previous

extern uint16_t spr_vram_slots[128]; // VRAM slots in sprite page

extern ZP uint16_t spr_front_count; // Rendered non-UI unsorted front-forced sprites this frame
extern NEAR struct spr_queue_entry spr_queue_front[SPR_COUNT_MAX_FRONT];
extern ZP uint16_t spr_back_count; // Rendered non-UI unsorted back-forced sprites this frame (e.g. shadows)
extern NEAR struct spr_queue_entry spr_queue_back[SPR_COUNT_MAX_BACK];

extern ZP uint16_t spr_normal_count;
extern NEAR uint8_t spr_depth_count[257]; // Count of sprites on each depth line
extern NEAR struct spr_queue_entry spr_queue_normal[SPR_COUNT_MAX_SORTED]; // depth sorted sprite entries
